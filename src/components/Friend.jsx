...
const Friend = ({ friend }) => {
  ...
  console.log(friend);
  return (
    <div className="shadow rounded-xl p-5 space-y-2 flex flex-col justify-center items-center">
      <img className="rounded-full" src={picture} alt={name} />
      <h1 className="font-semibold text-2xl">{name}</h1>
      <p className="text-[#64748B]">{days_since_contact}d ago</p>
      <div>
        {tags.map((tag) => (
          <span className="px-2 py-1 mx-2   bg-green-200 rounded-3xl font-medium">
            {tag.toUpperCase()}
          </span>
        ))}
      </div>
